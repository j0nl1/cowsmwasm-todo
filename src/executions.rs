...
pub fn add_todo(deps: DepsMut, info: MessageInfo, description: String) -> Result<Response, ContractError> {
    let data = Todo { description: description, status: Status::OPEN };
    let id = get_id(deps.storage)?;
    TODOS.save(deps.storage, (info.sender.clone(), id), &data)?;
    Ok(Response::new().add_attribute("method", "try_add").add_attribute("todo_id", "1"))
}

pub fn change_status(deps: DepsMut, info: MessageInfo, id: u64, status: u8) -> Result<Response, ContractError> {
